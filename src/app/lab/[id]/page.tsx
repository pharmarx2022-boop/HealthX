...
export default function LabDetailPage() {
    ...
    return (
        <div className="flex flex-col min-h-screen">
            ...
            <main className="flex-1 bg-slate-50/50 flex items-center justify-center">
                ...
                <p className="ml-4 text-muted-foreground">Loading lab profile...</p>
            </main>
            <Footer />
        </div>
    );
  ...
  const totalReviews = lab.reviewsList?.length ?? 0;
  const averageRating = totalReviews > 0
    ? (lab.reviewsList.reduce((acc, review) => acc + review.rating, 0) / totalReviews).toFixed(1)
    : 'N/A';
            ...
            <Card className="max-w-4xl mx-auto shadow-lg">
                ...
                <CardHeader>
                    <div className="flex justify-between items-start">
                        <CardTitle className="font-headline text-3xl">{lab.name}</CardTitle>
                        <Badge className={isOpen ? 'bg-green-600 text-white' : 'bg-destructive text-white'}>
                            <Clock className="w-4 h-4 mr-2"/> {text}
                        </Badge>
                    </div>
                ...
                <CardContent className="space-y-6">
                    ...
                    <div className="flex flex-wrap gap-4">
                        ...
                        {lab.homeCollectionEnabled && (
                            <Badge variant="secondary">
                                <Truck className="mr-2 text-primary" /> Home Collection (up to {lab.collectionRadius}km)
                            </Badge>
                        )}
                    </div>
                   
                    <Button asChild size="lg" className="w-full mt-4">
                        <a href={`https://wa.me/${lab.phoneNumber}`} target="_blank" rel="noopener noreferrer">
                            <WhatsAppIcon />
                 ...
                 <CardContent className="p-6">
                    ...
                    {lab.reviewsList && lab.reviewsList.length > 0 ? (
                        <div className="space-y-6">
                            {lab.reviewsList.map((review, index) => (
